 ...
 *
 * Return values:
 *   -1  control characters (C0/C1, except NUL handled as 0 by most TUIs — we
 *        keep -1 for clarity; callers like tui/text.lua should treat anything
 *        < 0 as "skip"/"substitute").
 *    0  zero-width: combining marks, ZWJ/ZWNJ, variation selectors, default-
 *        ignorable format chars. Also NUL.
 *    1  normal narrow character.
 *    2  East Asian Wide / Fullwidth, and Emoji_Presentation characters.
 *
 * The tables below are a curated subset sufficient for a chat TUI: CJK ideo-
 * graphs, Hangul syllables, fullwidth forms, major emoji blocks, and the
 * commonly-encountered zero-width ranges (combining marks, formatting chars,
 * variation selectors). We do NOT pull in utf8proc — this is ~120 entries
 * total, compiled straight into the binary.
 *
 * Data sourced from Unicode 15.1 EastAsianWidth.txt (W+F) and emoji-data.txt
 * (Emoji_Presentation). Kept as sorted {lo, hi} pairs for binary search.
 */

typedef struct { uint32_t lo, hi; } range_t;

/* -------------------------------------------------------------------------- */
/* Zero-width ranges (combining marks + format + variation selectors).        */
/* -------------------------------------------------------------------------- */

static const range_t zero_ranges[] = {
    ...
    {0xFB1E, 0xFB1E}, {0xFE00, 0xFE0F},  /* Variation Selectors 1-16 */
    {0xFE20, 0xFE2F}, {0xFEFF, 0xFEFF},  /* ZWNBSP / BOM */
    {0xFFF9, 0xFFFB},
    ...
    {0x112DF, 0x112DF}, {0x112E3, 0x112EA},
    ...
    {0x1E130, 0x1E136}, {0x1E2EC, 0x1E2EF},
    {0x1E8D0, 0x1E8D6}, {0x1E944, 0x1E94A},
    {0xE0001, 0xE0001}, {0xE0020, 0xE007F}, {0xE0100, 0xE01EF},
};

/* -------------------------------------------------------------------------- */
/* Wide ranges (East Asian Wide / Fullwidth + Emoji_Presentation).            */
/* -------------------------------------------------------------------------- */
...
static const range_t wide_ranges[] = {
    ...
    {0x3041, 0x33FF},  /* Hiragana + Katakana + CJK Compat + Ext */
    ...
    {0x1FAF0, 0x1FAF8},
    {0x20000, 0x2FFFD}, /* CJK Ext B-F + supplementary ideographs */
    {0x30000, 0x3FFFD},
};
...
static int
in_range(uint32_t cp, const range_t *ranges, int n) {
    int lo = 0, hi = n - 1;
    while (lo <= hi) {
        int mid = (lo + hi) >> 1;
...
wcwidth_cp(uint32_t cp) {
    ...
    if (cp >= 0x7F && cp < 0xA0) return -1;       /* DEL + C1 control */
    if (in_range(cp, zero_ranges, ARRAY_LEN(zero_ranges))) return 0;
    if (in_range(cp, wide_ranges, ARRAY_LEN(wide_ranges))) return 2;
    return 1;
}
...
utf8_next(const unsigned char *s, size_t n, size_t *out_i) {
    ...
    else if (b < 0xC2) { *out_i = i + 1;  return 0xFFFD; }
    else if (b < 0xE0) { cp = b & 0x1F;   extra = 1; }
    else if (b < 0xF0) { cp = b & 0x0F;   extra = 2; }
    else if (b < 0xF5) { cp = b & 0x07;   extra = 3; }
    else               { *out_i = i + 1;  return 0xFFFD; }

    if (i + 1 + extra > n) { *out_i = n; return 0xFFFD; }
    for (int k = 1; k <= extra; k++) {
        unsigned char c = s[i + k];
        if ((c & 0xC0) != 0x80) { *out_i = i + k; return 0xFFFD; }
        cp = (cp << 6) | (c & 0x3F);
    }
    *out_i = i + 1 + extra;
    return cp;
...
static int
l_wcwidth(lua_State *L) {
    lua_Integer cp = luaL_checkinteger(L, 1);
    if (cp < 0 || cp > 0x10FFFF) { lua_pushinteger(L, -1); return 1; }
    lua_pushinteger(L, wcwidth_cp((uint32_t)cp));
    return 1;
}
...
l_char_width(lua_State *L) {
    ...
    if (i >= n) { lua_pushinteger(L, 0); lua_pushinteger(L, (lua_Integer)(n + 1)); return 2; }
    uint32_t cp = utf8_next((const unsigned char *)s, n, &i);
