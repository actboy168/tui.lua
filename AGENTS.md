...
AI 在以下情况必须更新 `docs/decisions/`：

**必须新增决策：**
- 用户明确说"记下来"、"这是决策"、"以后要记住"
- 讨论后选择了方案 A 而非方案 B，且理由非显而易见
- 引入新的外部约束（平台、工具链、上游库限制）
- 定义新的阈值/常量，且需要说明为何取这个值

**必须更新决策：**
- 代码改动使某决策过时（更新或删除）
- 用户纠正了之前的决策（标记废弃并说明新决策）

**禁止写入：**
- 功能清单（代码已实现）
- 一次性的 bug 修复（除非涉及设计权衡）
- 纯代码细节（函数名、行号）

**格式：**
- 文件名反映主题（如 `focus_system.md`、`error_boundary.md`）
- 每篇包含：决策内容、Why（原因）、How to apply（如何应用）
- 更新后同步修改 `_index.md` 索引
